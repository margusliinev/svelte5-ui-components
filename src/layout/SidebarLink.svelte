<script lang="ts">
    ...
    let { children, toggleSidebar, route, href, ...props }: SidebarLinkProps = $props();
...
<li>
    <a {...props} {href} class={`${sidebarLinkStyles} ${route === href ? 'bg-secondary/20' : ''}`} onclick={toggleSidebar} role="button" tabindex="0">
        {@render children()}
    </a>
</li>
